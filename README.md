...
PUBLIC image CDN repo for CGK Linens (cgk.com). Everything in this repo is
consumed via
`https://raw.githubusercontent.com/cgk-platform/cgk-adimages/main/<path>`
...
|---|---|---|
| `brand/` | CGK Linens logo variants (PNG + SVG) and palette card | Seeded at setup from the brand kit; updated manually |
...
| `emails/<slug>-<sha8>.png\|jpg` | Rendered screenshots of publicly sent CGK Linens emails, content-addressed | ESP render lanes (module: klaviyo) |
...
| `social/<media_id>.jpg` | Normalized cover/poster for a public first-party CGK Linens Instagram post (feed, carousel, video, or story frame) | Organic-social visual sync (module: adimages-cdn) |
...
- Producers commit with machine identities and push immediately after
  ...
  are concerned.
- `social/` contains only media owned and publicly posted by CGK Linens — the
  capture lane filters on CGK Linens's own Instagram handle. It excludes DMs,
